...
class PaLM {
	...
	public constructor(key: string, rawConfig: { fetch?: typeof fetch } = {}) {
		...
		if (!config.fetch) {
			throw new Error(
				"Fetch was not found in environment, and no polyfill was provided. Please install a polyfill, and put it in the `fetch` property of the PaLM configuration."
			);
		}
	 ...
	 */
	private async query<K extends keyof QueryType>(
		model: string,
		command: K,
		body: object
	): Promise<QueryType[K]> {
		...
		const response = await this.#fetch(
			`https://generativelanguage.googleapis.com/v1beta2/models/${model}:${command}?key=${this.key}`,
			opts
		);
	 ...
	 */
	public async generateText<F extends Format = "markdown">(
		message: string,
		rawConfig: Partial<GenerateTextConfig<F>> = {}
	): Promise<ResponseByFormat<GenerateTextResponse>[F]> {
		...
		switch (config.format) {
			case PaLM.FORMATS.MD:
				return response.candidates[0]
					.output as ResponseByFormat<GenerateTextResponse>[F];
			case PaLM.FORMATS.JSON:
				return response as ResponseByFormat<GenerateTextResponse>[F];
			default:
				throw new Error(
					`${config.format} is not a valid format. Use PaLM.FORMATS.MD or PaLM.FORMATS.JSON.`
				);
		}
	}
	 ...
	 */
	public async ask<F extends Format = "markdown">(
		message: string,
		rawConfig: Partial<AskConfig<F>> = {}
	): Promise<ResponseByFormat<AskResponse>[F]> {
		...
		try {
			switch (config.format) {
				case PaLM.FORMATS.MD:
					return response.candidates[0]
						.content as ResponseByFormat<AskResponse>[F];
				case PaLM.FORMATS.JSON:
			...
			throw new Error(
				`Request rejected. Got ${response.filters} instead of response.`
			);
		}
	}
	...
	public async embed(
		message: string,
		rawConfig: Partial<EmbedTextConfig> = {}
	): Promise<number[]> {
		const config = this.parseConfig<EmbedTextConfig>(
...
class Chat {
	 ...
	 * @returns {Promise<ResponseByFormat<AskResponse>[F]>}
	 */
	async ask<F extends Format = "markdown">(
		message: string,
		rawConfig: Partial<ChatAskConfig<F>> = {}
	): Promise<ResponseByFormat<AskResponse>[F]> {
		const config = {
			...this.config,
			...this.PaLM["parseConfig"]<ChatAskConfig<Format>>(
				{ format: PaLM.FORMATS.MD },
				rawConfig
			),
		};
